...
for (i,m) in enumerate(mRange)
    ...
    problem = create_SteadyProblem(model=PazyWingTorsionTest)
    solve!(problem)
...
display(plt1)
savefig(string(pwd(),"/test/outputs/figures/PazyWingTorsionTest_1.pdf"))
...
display(plt2)
savefig(string(pwd(),"/test/outputs/figures/PazyWingTorsionTest_2.pdf"))
