...
plt1 = plot()
plot!(tNorm,u₃_mid/δ, c=:black, linewidth=2, xlabel="\$t/T\$", ylabel="\$u_3/\\delta\$ at \$x_1=L/2\$", label="Numerical")
scatter!(tNorm[1:5:end],u₃_mid_analytic[1:5:end]/δ, c=:blue, markersize=3, label="Analytical")
display(plt1)
savefig(string(pwd(),"/test/outputs/figures/biclampedBeam_1.pdf"))
# Normalized velocity at mid-length 
plt2 = plot()
plot!(tNorm,V₃_mid/δ, c=:black, linewidth=2, xlabel="\$t/T\$", ylabel="\$V_3/\\delta\$ at \$x_1=L/2\$ [\$1\$/s]", label="Numerical")
scatter!(tNorm[1:5:end],V₃_mid_analytic[1:5:end]/δ, c=:blue, markersize=3, label="Analytical")
display(plt2)
savefig(string(pwd(),"/test/outputs/figures/biclampedBeam_2.pdf"))
# Normalized acceleration at mid-length
plt3 = plot()
plot!(tNorm,Vdot₃_mid/δ, c=:black, linewidth=2, xlabel="\$t/T\$", ylabel="\$\\dot{V}_3/\\delta\$ at \$x_1=L/2\$ [\$1\$/\$s^2\$]", label="Numerical")
scatter!(tNorm[1:5:end],Vdot₃_mid_analytic[1:5:end]/δ, c=:blue, markersize=3, label="Analytical")
display(plt3)
savefig(string(pwd(),"/test/outputs/figures/biclampedBeam_3.pdf"))
# Normalized rotation at quarter-length
plt4 = plot()
plot!(tNorm,θ₂_quarter/(2*π)/δ, c=:black, linewidth=2, xlabel="\$t/T\$", ylabel="\$\\theta/(2\\pi\\delta)\$ at \$x_1=L/4\$", label="Numerical")
scatter!(tNorm[1:5:end],θ₂_quarter_analytic[1:5:end]/(2*π)/δ, c=:blue, markersize=3, label="Analytical")
display(plt4)
savefig(string(pwd(),"/test/outputs/figures/biclampedBeam_4.pdf"))
# Angular velocity at quarter-length
plt5 = plot()
plot!(tNorm,Ω₂_quarter, c=:black, linewidth=2, xlabel="\$t/T\$", ylabel="\$\\Omega_2\$ at \$x_1=L/4\$ [rad/s]", label="Numerical")
scatter!(tNorm[1:5:end],Ω₂_quarter_analytic[1:5:end], c=:blue, markersize=3, label="Analytical")
display(plt5)
savefig(string(pwd(),"/test/outputs/figures/biclampedBeam_5.pdf"))
# Angular acceleration at quarter-length
plt6 = plot()
plot!(tNorm,Ωdot₂_quarter, c=:black, linewidth=2, xlabel="\$t/T\$", ylabel="\$\\dot{\\Omega}_2\$ at \$x_1=L/4\$ [rad/\$s^2\$]", label="Numerical")
scatter!(tNorm[1:5:end],Ωdot₂_quarter_analytic[1:5:end], c=:blue, markersize=3, label="Analytical")
display(plt6)
savefig(string(pwd(),"/test/outputs/figures/biclampedBeam_6.pdf"))
