...
beam1 = Beam(name="beam1",length=L,nElements=nElem,C=[stiffnessMatrix1],rotationParametrization="E321")
...
beam2 = Beam(name="beam2",length=L,nElements=nElem,C=[stiffnessMatrix2],rotationParametrization="E321")

# Beam 3
stiffnessMatrix3 = diagm([3.9e6,1.1e6,1.2e5,1.18,0.983,290])
stiffnessMatrix3[1,4] = stiffnessMatrix3[4,1] = -522
beam3 = Beam(name="beam3",length=L,nElements=nElem,C=[stiffnessMatrix3],rotationParametrization="E321")
...
# Models
compositeCantilever1 = Model(name="compositeCantilever1",beams=[beam1],BCs=[clamp1,tipLoad1])
compositeCantilever2 = Model(name="compositeCantilever2",beams=[beam2],BCs=[clamp2,tipLoad2])
compositeCantilever3 = Model(name="compositeCantilever3",beams=[beam3],BCs=[clamp3,tipLoad3])

# Set system solver options
σ0 = 0
σstep = 0.1
NR = NewtonRaphson(initialLoadFactor=σ0,maximumLoadFactorStep=σstep,minimumLoadFactorStep=σstep)
...
for (j,θ) in enumerate(θRange)
    ...
    problem1 = SteadyProblem(model=compositeCantilever1,systemSolver=NR)
    problem2 = SteadyProblem(model=compositeCantilever2,systemSolver=NR)
    problem3 = SteadyProblem(model=compositeCantilever3,systemSolver=NR)
    solve!(problem1)
    solve!(problem2)
    solve!(problem3)
    # Save outputs
