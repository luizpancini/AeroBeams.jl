...
# Initialize model
HealyFFWTsteadyFlareRangeAoARangeCoast = create_HealyFFWT(flareAngle=0,kSpring=kSpring,pitchAngle=0,withTipCorrection=withTipCorrection,tipLossDecayFactor=tipLossDecayFactor,nElementsInner=nElementsInner,nElementsFFWT=nElementsFFWT)
...
r_n1 = [HealyFFWTsteadyFlareRangeAoARangeCoast.r_n[n][1] for n in elemNodes]
r_n2 = [HealyFFWTsteadyFlareRangeAoARangeCoast.r_n[n][2] for n in elemNodes]
r_n3 = [HealyFFWTsteadyFlareRangeAoARangeCoast.r_n[n][3] for n in elemNodes]
x1_n = vcat([vcat(HealyFFWTsteadyFlareRangeAoARangeCoast.beams[1].elements[e].x1_n1,HealyFFWTsteadyFlareRangeAoARangeCoast.beams[1].elements[e].x1_n2) for e in 1:15]...)
x1_e = [HealyFFWTsteadyFlareRangeAoARangeCoast.beams[1].elements[e].x1 for e in 1:15]
...
# Load reference data
flare10_exp = readdlm(pkgdir(AeroBeams)*"/test/referenceData/HealyFFWTsteadyFlareRangeAoARangeCoast/flare10_exp.txt")
flare10_num = readdlm(pkgdir(AeroBeams)*"/test/referenceData/HealyFFWTsteadyFlareRangeAoARangeCoast/flare10_num.txt")
flare20_exp = readdlm(pkgdir(AeroBeams)*"/test/referenceData/HealyFFWTsteadyFlareRangeAoARangeCoast/flare20_exp.txt")
flare20_num = readdlm(pkgdir(AeroBeams)*"/test/referenceData/HealyFFWTsteadyFlareRangeAoARangeCoast/flare20_num.txt")
flare30_exp = readdlm(pkgdir(AeroBeams)*"/test/referenceData/HealyFFWTsteadyFlareRangeAoARangeCoast/flare30_exp.txt")
flare30_num = readdlm(pkgdir(AeroBeams)*"/test/referenceData/HealyFFWTsteadyFlareRangeAoARangeCoast/flare30_num.txt")

println("Finished HealyFFWTsteadyFlareRangeAoARangeCoast.jl")