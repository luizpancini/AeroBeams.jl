...
# Model
sweptWingBendingTorsionalDivergence = create_Model(name="sweptWingBendingTorsionalDivergence",beams=[wing],BCs=[clamp],atmosphere=atmosphere)
...
for (i,U) in enumerate(URange)
    ...
    set_motion_basis_A!(model=sweptWingBendingTorsionalDivergence,v_A=[0;U;0])
...
qDRef = 19/3*EIy/(2π*c*L^3)*(1+tan(Λ)^2)/(tan(Λ∞)-tan(Λ))
...
println("Finished sweptWingBendingTorsionalDivergence.jl")