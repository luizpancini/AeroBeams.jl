...
# Sweep angle range (bending divergence only occurs for negative (forward) sweep)
ΛRange = π/180*vcat(-60:5:-5)
...
UDRef = @. sqrt(2*-19/3*EIy/(ρ*c*2π*L^3*sin(ΛRange)*cos(ΛRange)))

println("Finished sweptWingBendingDivergenceSweepRange.jl")