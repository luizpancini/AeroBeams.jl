...
#md plt1 = plot(xlabel="Airspeed [m/s]", ylabel="Fold angle [deg]", xlims=[0,40], ylims=[-90,30], yticks=-90:30:30, title="Without tip loss")
#md plot!([NaN],[NaN], lc=:black, ls=:dash, lw=lw, label="Healy (2023) - ST")
#md plot!([NaN],[NaN], lc=:black, ls=:solid, lw=lw, label="AeroBeams")
#md for (j,θ) in enumerate(θRange)
#md     plot!(fold_ST[2*j-1,:],fold_ST[2*j,:], c=colors[j], ls=:dash, lw=lw, label=false)
#md     plot!(URange, ϕHinge[2,j,:], c=colors[j], lw=lw, label="\$\\theta = $(round(Int,θ*180/π)) \\degree\$")
#md end
#md savefig("HealyBaselineFFWTfreeFlutterAoARangeURange_fold_tipLoss0.svg")

## Fold angle - with tip loss
#md plt2 = plot(xlabel="Airspeed [m/s]", ylabel="Fold angle [deg]", xlims=[0,40], ylims=[-90,30], yticks=-90:30:30, title="With tip loss")
#md plot!([NaN],[NaN], lc=:black, ls=:dash, lw=lw, label="Healy (2023) - VLM")
#md plot!([NaN],[NaN], lc=:black, ls=:solid, lw=lw, label="AeroBeams")
#md for (j,θ) in enumerate(θRange)
#md     plot!(fold_VLM[2*j-1,:],fold_VLM[2*j,:], c=colors[j], ls=:dash, lw=lw, label=false)
#md     plot!(URange, ϕHinge[1,j,:], c=colors[j], lw=lw, label="\$\\theta = $(round(Int,θ*180/π)) \\degree\$")
#md end
...
#md         plot!(plt31, URange, modeFrequencies[2,j,mode]/(2*π), c=colors[j], lw=lw, label=false)
#md         plot!(plt32, URange, modeDampingRatios[2,j,mode], c=colors[j], lw=lw, label=false)
#md     end
...
#md     plot!(plt32, damp1_ST[2*j-1,:], damp1_ST[2*j,:], c=colors[j], ls=:dash, lw=lw, label=false)
#md     plot!(plt32, damp2_ST[2*j-1,:], damp2_ST[2*j,:], c=colors[j], ls=:dash, lw=lw, label=false)
#md     plot!(plt32,[NaN],[NaN], c=colors[j], ls=:solid, lw=lw, label="\$\\theta = $(round(Int,θ*180/π)) \\degree\$")
#md end
#md plt3 = plot(plt31,plt32, layout=(2,1))
#md savefig("HealyBaselineFFWTfreeFlutterAoARangeURange_Vgf_tipLoss0.svg")

## V-g-f - with tip loss
#md plt41 = plot(ylabel="Frequency [Hz]", xlims=[0,30], ylims=[0,5], title="With tip loss", legend=:bottomright)
#md plt42 = plot(xlabel="Airspeed [m/s]", ylabel="Damping Ratio", xlims=[0,30], ylims=[-0.5,0.25], legend=:bottomleft)
#md plot!(plt41, [NaN],[NaN], lc=:black, ls=:dash, lw=lw, label="Healy (2023) - VLM")
#md plot!(plt41, [NaN],[NaN], lc=:black, ls=:solid, lw=lw, label="AeroBeams")
#md for (j,θ) in enumerate(θRange)
#md     for mode in 1:nModes
#md         plot!(plt41, URange, modeFrequencies[1,j,mode]/(2*π), c=colors[j], lw=lw, label=false)
#md         plot!(plt42, URange, modeDampingRatios[1,j,mode], c=colors[j], lw=lw, label=false)
#md     end
#md     plot!(plt41, freq1_VLM[2*j-1,:], freq1_VLM[2*j,:], c=colors[j], ls=:dash, lw=lw, label=false)
#md     plot!(plt41, freq2_VLM[2*j-1,:], freq2_VLM[2*j,:], c=colors[j], ls=:dash, lw=lw, label=false)
#md     plot!(plt42, damp1_VLM[2*j-1,:], damp1_VLM[2*j,:], c=colors[j], ls=:dash, lw=lw, label=false)
#md     plot!(plt42, damp2_VLM[2*j-1,:], damp2_VLM[2*j,:], c=colors[j], ls=:dash, lw=lw, label=false)
#md     plot!(plt42,[NaN],[NaN], c=colors[j], ls=:solid, lw=lw, label="\$\\theta = $(round(Int,θ*180/π)) \\degree\$")
#md end
#md plt4 = plot(plt41,plt42, layout=(2,1))
#md savefig("HealyBaselineFFWTfreeFlutterAoARangeURange_Vgf_tipLoss1.svg")

#md nothing #hide

#md # ![](HealyBaselineFFWTfreeFlutterAoARangeURange_fold_tipLoss0.svg)
#md # ![](HealyBaselineFFWTfreeFlutterAoARangeURange_fold_tipLoss1.svg)
#md # ![](HealyBaselineFFWTfreeFlutterAoARangeURange_Vgf_tipLoss0.svg)
#md # ![](HealyBaselineFFWTfreeFlutterAoARangeURange_Vgf_tipLoss1.svg)

println("Finished HealyBaselineFFWTfreeFlutterAoARangeURange.jl") #src