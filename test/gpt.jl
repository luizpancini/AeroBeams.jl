...
# Constructor
function BC(;name::String="",beam::Int64,node::Int64,types::Vector{String},values,toBeTrimmed::Union{BitVector,Vector{Bool}}=falses(length(types)))
    ...
    #     if !(type in ["F1b","F2b","F3b","M1b","M2b","M3b","Ff1b","Ff2b","Ff3b","Mf1b","Mf2b","Mf3b"]) && toBeTrimmed[i]
    #         error("Trim variables must be loads specified on basis b")
    #     end
    # end
    # for value in values
    ...
    # Initialize BC
    bc = BC(name,beam,node,types,values,toBeTrimmed,Vector{Float64}(),BitVector(),BitVector(),BitVector(),I3,0)
...
F = 130e3
clamp = BC(name="clamp",beam=1,node=1,types=["u1A","u2A","u3A","p1A","p2A","p3A"],values=[0,0,0,0,0,0])