...
plot!(tNorm,u3_quarter, c=:black, lw=lw, label="Numerical")
scatter!(tNorm[1:5:end],u3_quarter_analytic[1:5:end], c=:blue, ms=ms, msw=0, label="Analytical")
display(plt1)
...
plot!(tNorm,V3_quarter, c=:black, lw=lw, label="Numerical")
scatter!(tNorm[1:5:end],V3_quarter_analytic[1:5:end], c=:blue, ms=ms, msw=0, label="Analytical")
display(plt2)
...
plot!(tNorm,Vdot3_quarter, c=:black, lw=lw, label="Numerical")
scatter!(tNorm[1:5:end],Vdot3_quarter_analytic[1:5:end], c=:blue, ms=ms, msw=0, label="Analytical")
display(plt3)
savefig(string(absPath,"/initialDispAndVelBeam_acc.pdf"))

# Rotation at root
plt4 = plot(xlabel="\$t/T\$", ylabel="\$\\theta/(2\\pi\\delta)\$ at \$x_1=0\$")
plot!(tNorm,θ2_root/(2*π), c=:black, lw=lw, label="Numerical")
scatter!(tNorm[1:5:end],θ2_root_analytic[1:5:end]/(2*π), c=:blue, ms=ms, msw=0, label="Analytical")
display(plt4)
