...
# Set paths
relPath = "/test/outputs/figures/coastingFoldingWingtip"
absPath = string(pwd(),relPath)
mkpath(absPath)

# Deformed shape
deformationPlot = plot_steady_deformation(problem,plotUndeformed=true,view=(15,15),plotDistLoads=true,save=true,savePath=string(relPath,"/coastingFoldingWingtip_deformation.pdf"))
...
display(plt_u3)
savefig(string(absPath,"/coastingFoldingWingtip_u3.pdf"))
...
display(plt_u2)
savefig(string(absPath,"/coastingFoldingWingtip_u2.pdf"))

# p1
plt3 = plot(x1_n/x1_n[end], p1, lw=lw, label=false, xlabel="\$x_1/L\$", ylabel="\$p_1\$")
display(plt3)
savefig(string(absPath,"/coastingFoldingWingtip_p1.pdf"))

# p2
plt2 = plot(x1_n/x1_n[end], p2, lw=lw, label=false, xlabel="\$x_1/L\$", ylabel="\$p_2\$")
display(plt2)
savefig(string(absPath,"/coastingFoldingWingtip_p2.pdf"))

# p3
plt2 = plot(x1_n/x1_n[end], p3, lw=lw, label=false, xlabel="\$x_1/L\$", ylabel="\$p_3\$")
display(plt2)
savefig(string(absPath,"/coastingFoldingWingtip_p3.pdf"))

# Shear force
plt3 = plot(x1_n/x1_n[end], F3, lw=lw, label=false, xlabel="\$x_1/L\$", ylabel="\$F_3\$ [N]")
display(plt3)
savefig(string(absPath,"/coastingFoldingWingtip_F3.pdf"))

# Bending moment
plt4 = plot(x1_n/x1_n[end], M2, lw=lw, label=false, xlabel="\$x_1/L\$", ylabel="\$M_2\$ [N.m]")
display(plt4)
savefig(string(absPath,"/coastingFoldingWingtip_M2.pdf"))

println("Finished coastingFoldingWingtipPlotGenerator.jl")