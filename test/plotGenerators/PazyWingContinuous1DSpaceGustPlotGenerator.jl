...
# Set paths
relPath = "/test/outputs/figures/PazyWingContinuous1DSpaceGust"
absPath = string(pwd(),relPath)
mkpath(absPath)

# Animation
anim = plot_dynamic_deformation(problem,refBasis="A",plotFrequency=10,plotLimits=([-L/2,L/2],[-L/2,L/2],[0,L]),save=true,savePath=string(relPath,"/PazyWingContinuous1DSpaceGust_deformation.gif"),displayProgress=true)
display(anim)
...
display(plt6)
savefig(string(absPath,"/PazyWingContinuous1DSpaceGust_states.pdf"))
...
display(plt7)
savefig(string(absPath,"/PazyWingContinuous1DSpaceGust_gust.pdf"))

println("Finished PazyWingContinuous1DSpaceGustPlotGenerator.jl")