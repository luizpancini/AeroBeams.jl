...
# Deformed shape
deformationPlot = plot_steady_deformation(problem,plotUndeformed=true,plotDistLoads=true,view=(60,30),legendPos=(0.3,0.5),save=true,savePath=string(relPath,"/PazyFFWTsteady_deformation.pdf"))
...
display(plt3)
savefig(string(absPath,"/PazyFFWTsteady_p1.pdf"))
...
display(plt2)
savefig(string(absPath,"/PazyFFWTsteady_p3.pdf"))
...
display(plt5)
savefig(string(absPath,"/PazyFFWTsteady_aoa.pdf"))

# cn
plt6 = plot(x1_e/x1_n[end], cn, lw=lw, label=false, xlabel="\$x_1/L\$", ylabel="\$c_n\$ ")
display(plt6)
savefig(string(absPath,"/PazyFFWTsteady_cn.pdf"))
