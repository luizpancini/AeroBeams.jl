...
for (i,U) in enumerate(URange)
    ...
    eigenProblem = create_EigenProblem(model=BWBeigen,nModes=nModes,frequencyFilterLimits=[1e0,Inf64],jacobian=trimProblem.jacobian[1:end,1:end-trimProblem.model.nTrimVariables],inertia=trimProblem.inertia)
...
display(plt1)
savefig(string(pwd(),"/test/outputs/figures/BWBstability_AoA.pdf"))
...
display(plt2)
savefig(string(pwd(),"/test/outputs/figures/BWBstability_thrust.pdf"))
...
display(plt3)
savefig(string(pwd(),"/test/outputs/figures/BWBstability_delta.pdf"))
# Root locus
...
display(plt4)
savefig(string(pwd(),"/test/outputs/figures/BWBstability_rootlocus.pdf"))
# V-g-f
...
display(plt5)
savefig(string(pwd(),"/test/outputs/figures/BWBstability_Vgf.pdf"))
