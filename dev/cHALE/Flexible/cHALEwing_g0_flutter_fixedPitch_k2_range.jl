...
# Mode shapes animation of wing with k_2=0.0 at lowest airspeed
modesAnim = plot_mode_shapes_animation(eigenProblem[2,1],scale=10,view=(60,15),legendPos=(0.2,0.6),modes2plot=[3,4],modeLabels=["T","IP"],nFramesPerCycle=21,plotSteady=true,plotBCs=true,plotAxes=false,modalColorScheme=colors,legendFontSize=10,plotLimits=([0,L],[-L/2,L/2],[-L/2,L/2]),save=true,savePath=string(relPath,"/cHALEwing_modeShapes_k0U1_lambda",λ,".gif"),displayProgress=true)
display(modesAnim)

# # Mode shapes animation of wing with k_2=0.045 at lowest airspeed
# modesAnim = plot_mode_shapes_animation(eigenProblem[end,1],scale=10,view=(60,15),legendPos=(0.2,0.6),modes2plot=[2,5],modeLabels=["T-IP-1","T-IP-2"],nFramesPerCycle=101,plotSteady=true,plotBCs=true,plotAxes=false,modalColorScheme=colors,legendFontSize=10,save=true,savePath=string(relPath,"/cHALEwing_modeShapes_k0045U1_lambda",λ,".gif"),displayProgress=true)
# display(modesAnim)

# # Steady plot of unloaded straight wing
# plt_steady = plot_steady_deformation(eigenProblem[2,1],backendSymbol=:gr,plotUndeformed=false,plotBCs=true,plotDistLoads=false,plotLegend=false,plotAxes=false,plotGrid=false,view=(30,30),plotLimits=([-L*1/4,L*3/4],[-L*1/2,L*1/2],[-L*1/2,L*1/2]),save=true,savePath=string(relPath,"/cHALEwing_steady_k0U0_lambda",λ,".pdf"))
# display(plt_steady)
