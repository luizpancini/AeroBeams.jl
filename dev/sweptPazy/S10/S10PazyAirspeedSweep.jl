...
if solveFlutter
    ...
    for (j,U) in enumerate(URange)
        ...
        flutterProblem[j] = create_EigenProblem(model=model,nModes=nModes,frequencyFilterLimits=[2π,Inf],x0=x0)
        ...
        ϵ11Root = flutterProblem[j].compElementalStatesOverσ[end][1].γ[1]
        κ2Root = flutterProblem[j].compElementalStatesOverσ[end][1].κ[2]
        κ3Root = flutterProblem[j].compElementalStatesOverσ[end][1].κ[3]
        rootEps_f[j] = (ϵ11Root .- κ2Root*zSG .- κ3Root*ySG)
    end
    ...
    # Plot V-g-f
    relPath = "/dev/sweptPazy/S10/outputs/S10PazyAirspeedSweep"
    ...
    display(plt_Vgf)
    savefig(plt_Vgf,string(absPath,"/S10PazyAirspeedSweep_",idString,"_Vgf.pdf"))
    gui(plt_Vgf)
end
...
# Model
model,_ = create_Pazy(aeroSolver=aeroSolver,airfoil=airfoil,upright=upright,Λ=Λ,θ=θ,airspeed=U,hasTipCorrection=hasTipCorrection,tipLossType=tipLossType,sweepStructuralCorrections=sweepStructuralCorrections,tipMass=tipMass,ηtipMass=[0;tipMassPos;0],tipLossFunctionIsAirspeedDependent=tipLossFunctionIsAirspeedDependent)

# Create and solve steady problem for initial airspeed
steadyProblem = create_SteadyProblem(model=model)
solve!(steadyProblem)

# Create and solve dynamic problem
dynamicProblem = create_DynamicProblem(model=model,finalTime=tf,Δt=Δt,systemSolver=NR,x0=steadyProblem.x,skipInitialStatesUpdate=true,trackingFrequency=trackingFrequency)
...
# Animation
plot_dynamic_deformation(dynamicProblem,refBasis="A",plotFrequency=round(Int,5e-2/Δt),plotLimits=([-L/2,L/2],[-L/2,L/2],[0,L]),save=true,savePath=string(relPath,"/S10PazyAirspeedSweep_",idString,"_deformation.gif"),displayProgress=true)
...
display(plt_U)
savefig(string(absPath,"/S10PazyAirspeedSweep_",idString,"_U.pdf"))
...
if showAirspeedInPlots
    ...
    plot!(plt_tipOOP_twin, ylabel="Airspeed [m/s]", tickfont=font(ts), guidefont=font(fs), extrema(t))
    plot!(plt_tipOOP_twin, t, U.(t), lw=lw, c=:black, label=false)
end
display(plt_tipOOP)
savefig(string(absPath,"/S10PazyAirspeedSweep_",idString,"_tipOOP.pdf"))
...
display(plt_tipAOA)
savefig(string(absPath,"/S10PazyAirspeedSweep_",idString,"_tipAoA.pdf"))
...
display(plt_rootEps)
savefig(string(absPath,"/S10PazyAirspeedSweep_",idString,"_rootEps.pdf"))
...
plt_eps_spectrum = heatmap(t_eps, f_eps, S_eps; ylims=[0,100], title="Root strains spectrogram", xlabel="Airspeed [m/s]", ylabel="Frequency [Hz]", c=:rainbow, colorbar_title="Power [dB]", aspect_ratio=:auto, tickfont=font(ts), guidefont=font(fs))
...
savefig(string(absPath,"/S10PazyAirspeedSweep_",idString,"_rootEpsSpectrogram.pdf"))

println("Finished S10PazyAirspeedSweep.jl")
