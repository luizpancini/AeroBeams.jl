...
for (i,frame) in enumerate(frames)
    cnRef[i] = readdlm(pkgdir(AeroBeams)*"/test/referenceData/GU/"*frame*"_cn.txt")
    cmRef[i] = readdlm(pkgdir(AeroBeams)*"/test/referenceData/GU/"*frame*"_cm.txt")
    ctRef[i] = readdlm(pkgdir(AeroBeams)*"/test/referenceData/GU/"*frame*"_ct.txt")
end
...
# Set paths
relPath = "/dev/helios/figures/airfoilLoads"
...
println("Finished airfoilLoads.jl")
