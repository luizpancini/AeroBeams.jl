...
x1_e_wing = Array{Vector{Float64}}(undef,length(k1Range))
x1_e_HT = Array{Vector{Float64}}(undef,length(k1Range))
...
αWing = Array{Vector{Float64}}(undef,length(k1Range),length(URange))
cnWing = Array{Vector{Float64}}(undef,length(k1Range),length(URange))
ctWing = Array{Vector{Float64}}(undef,length(k1Range),length(URange))
clWing = Array{Vector{Float64}}(undef,length(k1Range),length(URange))
cdWing = Array{Vector{Float64}}(undef,length(k1Range),length(URange))
cdHT = Array{Vector{Float64}}(undef,length(k1Range),length(URange))
...
for (i,k1) in enumerate(k1Range)
    ...
    for (j,U) in enumerate(URange)
        ...
        cHALEeigen,_,_,tailBoom,_ = create_conventional_HALE(aeroSolver=aeroSolver,altitude=h,airspeed=U,nElemWing=nElemWing,nElemTailBoom=nElemTailBoom,nElemHorzStabilizer=nElemHorzStabilizer,nElemVertStabilizer=nElemVertStabilizer,stiffnessFactor=λ,∞=1e12,stabilizersAero=stabilizersAero,includeVS=includeVS,wingCd0=wingCd0,stabsCd0=stabsCd0,δElev=trimδ[i,j],thrust=trimThrust[i,j],k1=k1,hasInducedDrag=hasInducedDrag)
        # Add springs
        add_springs_to_beam!(beam=tailBoom,springs=[spring1,spring2])
        # Update model
        ...
        eigenProblem[i,j] = create_EigenProblem(model=cHALEeigen,nModes=nModes,frequencyFilterLimits=[5e-2,Inf64],jacobian=trimProblem.jacobian[1:end,1:end-trimProblem.model.nTrimVariables],inertia=trimProblem.inertia)
...
# Set paths
relPath = "/dev/outputs/figures/cHALE_flutter_k1_range"
absPath = string(pwd(),relPath)
mkpath(absPath)

# Mode shapes at lowest airspeed
for (n,k1) in enumerate(k1Range)
    plt = plot_mode_shapes(eigenProblem[n,1],nModes=5,scale=5,view=(30,30),modalColorScheme=:rainbow,legendPos=:outertop,save=true,savePath=string(relPath,string("/cHALE_flutter_k1_range_modeShapesk1_",n,".pdf")))
    display(plt)
end
...
display(plt0)
savefig(string(absPath,"/cHALE_flutter_k1_range_rootlocus.pdf"))

# Root locus (zoom 1)
plt1 = plot(xlabel="Damping [1/s]", ylabel="Frequency [rad/s]", xlims=[-5,0], ylims=[0,6])
...
display(plt1)
savefig(string(absPath,"/cHALE_flutter_k1_range_rootlocus_zoom1.pdf"))

# Root locus (zoom 2)
plt2 = plot(xlabel="Damping [1/s]", ylabel="Frequency [rad/s]", xlims=[-0.25,0.05], ylims=[0,0.5], legend=:topleft)
scatter!([NaN], [NaN], c=:white, shape=:star8, ms=ms, msw=1, msα=1, msc=:black, markerstrokestyle=:solid, label=false)
for (i,k1) in enumerate(k1Range)
    scatter!([NaN], [NaN], c=colors[i], shape=mshape[i], ms=ms, msw=msw, label=false)
    for mode in 1:nModes
...
display(plt2)
savefig(string(absPath,"/cHALE_flutter_k1_range_rootlocus_zoom2.pdf"))
...
display(plt3)
savefig(string(absPath,"/cHALE_flutter_k1_range_Vgf.pdf"))
...
display(plt4)
savefig(string(absPath,"/cHALE_flutter_k1_range_disp.pdf"))
...
display(plt5)
savefig(string(absPath,"/cHALE_flutter_k1_range_AoA.pdf"))
...
display(plt6)
savefig(string(absPath,"/cHALE_flutter_k1_range_cl.pdf"))
...
display(plt7)
savefig(string(absPath,"/cHALE_flutter_k1_range_cd.pdf"))

# Cd over span at lowest airspeed
plt72 = plot(xlabel="Normalized span", ylabel="HT \$c_d\$ []", xlims=[0,1], title="Airspeed = $(URange[1]) m/s")
for (i,k1) in enumerate(k1Range)
    plot!((x1_e_HT[i] .- HTsemispan)/HTsemispan, cdHT[i,1], c=colors[i], lw=lw, label=labels[i])
end
display(plt72)
savefig(string(absPath,"/cHALE_flutter_k1_range_cdHT.pdf"))
...
display(plt8)
savefig(string(absPath,"/cHALE_flutter_k1_range_cn.pdf"))
...
display(plt9)
savefig(string(absPath,"/cHALE_flutter_k1_range_ct.pdf"))
...
# Trim thrust
pltT2 = plot(xlabel="Airspeed [m/s]", ylabel="Thrust [N]", xlims=[URange[1],URange[end]], ylims=[0,60])
for (i,k1) in enumerate(k1Range)
