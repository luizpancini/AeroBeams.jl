...
# Set root pitch angle [deg], twisting curvature and airspeed ranges
θRange = [0,5,10,15]
...
for (n,θ) in enumerate(θRange)
    ...
    for (i,k1) in enumerate(k1Range)
        ...
        for (j,U) in enumerate(URange)
            ...
            # Model
            wingModel,_ = create_SMW(aeroSolver=aeroSolver,airspeed=U,nElem=nElem,stiffnessFactor=λ,∞=1e12,altitude=h,θ=θ*π/180,k1=k1,cd0=cd0,hasInducedDrag=hasInducedDrag)
...
# Set paths
relPath = "/dev/outputs/figures/cHALEwing_flutter_fixedPitch_k1_range"
...
for (n,θ) in enumerate(θRange)
    ...
    # Root locus
    plt0 = plot(xlabel="Damping [1/s]", ylabel="Frequency [rad/s]", xlims=[-10,2], ylims=[0,60])
...
println("Finished cHALEwing_flutter_fixedPitch_k1_range.jl")