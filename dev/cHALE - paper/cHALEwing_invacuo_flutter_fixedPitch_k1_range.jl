...
# Set root pitch angle [deg], twist curvature and airspeed ranges
θRange = [5]
k1Range = range(-0.005,0.005,3)
URange = collect(10:0.5:35)

# Initialize outputs
untrackedFreqs = Array{Vector{Float64}}(undef,length(θRange),length(k1Range),length(URange))
untrackedDamps = Array{Vector{Float64}}(undef,length(θRange),length(k1Range),length(URange))
untrackedEigenvectors = Array{Matrix{ComplexF64}}(undef,length(θRange),length(k1Range),length(URange))
freqs = Array{Vector{Float64}}(undef,length(θRange),length(k1Range),length(URange))
damps = Array{Vector{Float64}}(undef,length(θRange),length(k1Range),length(URange))
modeDampings = Array{Vector{Float64}}(undef,length(θRange),length(k1Range),nModes)
modeFrequencies = Array{Vector{Float64}}(undef,length(θRange),length(k1Range),nModes)

x1_0 = Array{Vector{Float64}}(undef,length(θRange),length(k1Range))
x3_0 = Array{Vector{Float64}}(undef,length(θRange),length(k1Range))
x1_e = Array{Vector{Float64}}(undef,length(θRange),length(k1Range))
u1_of_x1 = Array{Vector{Float64}}(undef,length(θRange),length(k1Range),length(URange))
u3_of_x1 = Array{Vector{Float64}}(undef,length(θRange),length(k1Range),length(URange))
x1_def = Array{Vector{Float64}}(undef,length(θRange),length(k1Range),length(URange))
x3_def = Array{Vector{Float64}}(undef,length(θRange),length(k1Range),length(URange))
α = Array{Vector{Float64}}(undef,length(θRange),length(k1Range),length(URange))
cn = Array{Vector{Float64}}(undef,length(θRange),length(k1Range),length(URange))
ct = Array{Vector{Float64}}(undef,length(θRange),length(k1Range),length(URange))
cl = Array{Vector{Float64}}(undef,length(θRange),length(k1Range),length(URange))
cd = Array{Vector{Float64}}(undef,length(θRange),length(k1Range),length(URange))

eigenProblem = Array{EigenProblem}(undef,length(θRange),length(k1Range),length(URange))
...
for (n,θ) in enumerate(θRange)
    ...
    for (i,k1) in enumerate(k1Range)
        ...
        for (j,U) in enumerate(URange)
            println("Solving for θ = $θ deg, k1 = $k1, U = $U m/s")
            # Model
            wingModel,_ = create_SMW(aeroSolver=aeroSolver,airspeed=U,nElem=nElem,stiffnessFactor=λ,∞=1e12,altitude=h,g=g,θ=θ*π/180,k1=k1,cd0=cd0,hasInducedDrag=hasInducedDrag)
...
# Set paths
relPath = "/dev/outputs/figures/cHALEwing_invacuo_flutter_fixedPitch_k1_range"
...
for (n,θ) in enumerate(θRange)
    ...
    display(plt0)
    savefig(string(absPath,string("/cHALEwing_flutter_fixedPitch",round(Int,θ),"_k1_range_rootlocus.pdf")))
    ...
    display(plt3)
    savefig(string(absPath,string("/cHALEwing_flutter_fixedPitch",round(Int,θ),"_k1_range_Vgf.pdf")))

    # Normalized deformed span at highest airspeed
    plt4 = plot(xlabel="\$x_1/L\$", ylabel="\$x_3/L\$", xlims=[0,1], title="Airspeed = $(URange[end]) m/s")
    for (i,k1) in enumerate(k1Range)
        plot!(x1_def[n,i,end]/L, x3_def[n,i,end]/L, c=colors[i], lw=lw, label=labels[i])
    end
    display(plt4)
    savefig(string(absPath,string("/cHALEwing_flutter_fixedPitch",round(Int,θ),"_k1_range_disp.pdf")))

    # Angle of attack at highest airspeed
    plt5 = plot(xlabel="\$x_1/L\$", ylabel="\$\\alpha\$ [deg]", xlims=[0,1], title="Airspeed = $(URange[end]) m/s")
    for (i,k1) in enumerate(k1Range)
        plot!(x1_e[i]/L, α[n,i,end], c=colors[i], lw=lw, label=labels[i])
    end
    display(plt5)
    savefig(string(absPath,string("/cHALEwing_flutter_fixedPitch",round(Int,θ),"_k1_range_AoA.pdf")))
end

println("Finished cHALEwing_invacuo_flutter_fixedPitch_k1_range.jl")